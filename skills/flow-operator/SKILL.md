...
- Retry Patchbay dispatch transport failures.
...
Against Patchbay admin endpoints:

```bash
curl -H "Authorization: Bearer $PATCHBAY_ADMIN_TOKEN" https://patchbay.peezy.tech/flow-events
curl -H "Authorization: Bearer $PATCHBAY_ADMIN_TOKEN" https://patchbay.peezy.tech/flow-dispatches
curl -X POST -H "Authorization: Bearer $PATCHBAY_ADMIN_TOKEN" https://patchbay.peezy.tech/flow-events/<encoded-event-id>/retry
curl -X POST -H "Authorization: Bearer $PATCHBAY_ADMIN_TOKEN" https://patchbay.peezy.tech/flow-events/<encoded-event-id>/replay
```
...
- Use Patchbay `retry` for dispatch/network failures before backend acceptance.
- Use backend or Patchbay `replay` when the backend accepted the event but a run failed, blocked, or needs intervention.
